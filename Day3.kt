fun checkRow(row: String, r: Int, symbols: String): List<Pair<Int, Int>> {
    var n = 0
...
fun getSymbols(board: List<String>, symbols: String): List<Pair<Int, Int>> {
    var n = 0
    val list = mutableListOf<Pair<Int, Int>>()

    for (row in board) {
        list += checkRow(row, n, symbols)
        n += 1
    }
...
fun checkNextTo(board: List<String>, coord: Pair<Int, Int>): String {
    val x = coord.first
    val y = coord.second
    ...
    while (n >= 0 && board[y][n] in "1234567890") {
        n -= 1
    }

    n += 1 // First number

    while (n < board[y].length && board[y][n] in "1234567890") {
        fullNumber += board[y][n]
...
fun checkUnit(board: List<String>, coord: Pair<Int, Int>): String {
    val x = coord.first
    val y = coord.second

    return if (board[y][x] in "1234567890") {
        checkNextTo(board, coord)
    } else {
        "0"
    }
}

fun checkAround(board: List<String>, coord: Pair<Int, Int>): List<String> {
    val x = coord.first
    val y = coord.second

    val nums = mutableSetOf<String>()

    nums += checkUnit(board, x - 1 to y - 1)
    nums += checkUnit(board, x to y - 1)
    nums += checkUnit(board, x + 1 to y - 1)
    nums += checkUnit(board, x - 1 to y)
    nums += checkUnit(board, x + 1 to y)
    nums += checkUnit(board, x - 1 to y + 1)
    nums += checkUnit(board, x to y + 1)
    nums += checkUnit(board, x + 1 to y + 1)
...
fun main() {
    ...
    // PART ONE
    val symbolCoords = getSymbols(board, "*/&=@+\$#-\$%")
    val nums = symbolCoords.map { sC -> checkAround(board, sC) }
    val n2 = nums.map { it.map { n -> n.toInt() } }
    println(n2.map { it.sum() }.sum())

    // PART TWO
    val symbolCoords2 = getSymbols(board, "*")
    val nums2 = symbolCoords2.map { sC -> checkAround(board, sC) }
    val gears = nums2.filter { it.size == 3 }

    val cleanGears = mutableListOf<List<String>>()

    for (gear in gears) {
        cleanGears.add( gear.filter { it != "0" } )
    }

    val numGears = cleanGears.map { it.map { n -> n.toInt() } }
    val prods = numGears.map { it[0] * it[1] }
    println(prods.sum())
}
